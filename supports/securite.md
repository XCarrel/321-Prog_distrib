...
Une clé de chiffrement est une donnée secrète  qui est utilisé par un algorithme pour chiffrer ou déchiffrer une donnée. L'algorithme de chiffrement reçoit en entrée une donnée en clair et la clé de chiffrement, il produit un résultat que l'on appelle donnée chiffrée. Lorsqu'on fournit la données chiffrée et une clé à un algorithme de déchiffrement, ce dernier réalise l'opération inverse et restitue la donnée en clair initiale.
...
Un algorithme asymétrique est un algorithme qui utilise une paire de clés publique/privée pour les opérations du chiffrement et de déchiffrement. Les deux clés sont intimement liées. Les clés ne sont pas destinées à des opérations fixes; en d'autre termes, on peut chiffrer avec une clé privée ou publique, et on on peut déchiffrer avec une clé privée ou publique. C'est l'usage qui dicte quelle clé est utilisée. Une donnée qui a été chiffrée avec la clé privée ne peut être déchiffrée que par la clé publique (elle ne peut même pas être déchiffrée par la même clé privée qui a été utilisée pour le chiffrement). Inversement, toute donnée qui a été chiffrée avec la clé publique ne peut être déchiffrer que par la clé privée de la même paire de clés.
## Hash
...
## Signature
Une signature est un élément cryptographique qui authentifie la source d'une donnée est en valide également son intégrité. Elle est construite ou deux temps :
- calcule du hash de la donnée, lequel permettra de valider l'intégrité de la donnée par le récepteur.
...
## Clé publique
La clé publique d'une paire de clé est celle qui peut être diffusée librement à toute personne ou organisation avec laquelle le détenteur de la cléprivée veut effectuer des échanges chiffrés.
